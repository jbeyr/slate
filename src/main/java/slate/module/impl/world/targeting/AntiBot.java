...
public class AntiBot extends Module {
    ...
    private static @NotNull List<String> getTablist() {
        return Main.mc.getNetHandler().getPlayerInfoMap().parallelStream()
                .map(NetworkPlayerInfo::getGameProfile)
                .filter(profile -> profile.getId() != Main.mc.thePlayer.getUniqueID())
    ...
    public void onUpdate() {
        ...
        for (EntityPlayer p : mc.theWorld.playerEntities) {
            ...
            if (lastPlayers.containsKey(name)) {
                ...
                Vec3 thePlayer = mc.thePlayer.getPositionVector();
                double existsDistance = thePlayer.distanceTo(exists.getPositionVector());
                double curDistance = thePlayer.distanceTo(p.getPositionVector());
