package slate.module.impl.combat.morekb;
...
public class SimpleSprintReset extends SubMode<IMoreKB> {
    ...
    public void onAttack(AttackEntityEvent event) {
        if (!Utils.nullCheckPasses() || event.entityPlayer != mc.thePlayer || delayTicksLeft > 0) return;
        ...
        if (!tm.isRecommendedTarget(event.target)/*AntiBot.isBot(event.target)*/) return;
