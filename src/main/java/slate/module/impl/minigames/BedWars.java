package slate.module.impl.minigames;
...
public class BedWars extends Module {
    ...
    public void onBlock(BlockEvent.PlaceEvent e) {
        if (!Utils.nullCheckPasses() || !obsidian.isToggled()) {
            return;
        }
    ...
    public void onEntityJoinWorld(EntityJoinWorldEvent e) {
        if (!Utils.nullCheckPasses() || e.entity == null) {
            return;
        }
            ...
            if (Utils.getBedwarsStatus() == 2) {
                if (diamondArmor.isToggled() || enderPearl.isToggled() || obsidian.isToggled()) {
                    for (EntityPlayer p : mc.theWorld.playerEntities) {
                        ...
                        if (!ModuleManager.targetManager.isRecommendedTarget(p)) {
                            continue;
                        }
