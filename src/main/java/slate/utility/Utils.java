...
import slate.mixins.impl.client.GuiScreenAccessor;
// import slate.module.impl.other.NameHider;
// import slate.module.impl.other.SlotHandler;
// import slate.module.impl.render.AntiShuffle;
...
public class Utils {
    ...
    public static boolean isFriended(EntityLivingBase elb) {
        if(!(elb instanceof EntityPlayer)) return false;
        return isFriended((EntityPlayer)elb);
    }
    ...
    public static Vec3 getEyePos(Entity entity) {
        return getEyePos(entity, entity.getPositionVector());
    }
    ...
    public static boolean isTargetNearby(double dist) {
        return mc.theWorld.playerEntities.stream()
                ...
                .anyMatch(target -> target.getPositionVector().distanceTo(mc.thePlayer.getPositionVector()) < dist);
    }
    ...
    // region Main BS utils for aim assist

    public static float getYaw(Entity ent) {
        double x = ent.posX - mc.thePlayer.posX;
        double z = ent.posZ - mc.thePlayer.posZ;
        double yaw = Math.atan2(x, z) * 57.29577951308232; // = (180/Math.PI); conversion factor from radians -> degrees
        return (float) (yaw * -1.0D);
    }

    public static float getYawForVec(Vec3 vec) {
        double dx = vec.xCoord - mc.thePlayer.posX;
        double dz = vec.zCoord - mc.thePlayer.posZ;
        double yaw = Math.atan2(dx, dz) * 57.29577951308232;
        return (float) (yaw * -1d);
    }

    /**
     * Calculates the yaw difference between the player and the given entity.
     *
     * @param en The entity to calculate the yaw difference to
     * @return The yaw difference in degrees, normalized to be between -180 and 180
     */
    public static double getYawDifference(Entity en) {
        return ((double) (mc.thePlayer.rotationYaw - getYaw(en)) % 360.0D + 540.0D) % 360.0D - 180.0D;
    }

    /**
     * Calculates the yaw difference between the player's position and the given vector.
     *
     * @param v The vector to calculate the yaw difference to
     * @return The yaw difference in degrees, normalized to be between -180 and 180
     */
    public static double getYawDifferenceForVec(Vec3 v) {
        return ((double) (mc.thePlayer.rotationYaw - getYawForVec(v)) % 360.0D + 540.0D) % 360.0D - 180.0D;
    }

    /**
     * Calculates the pitch difference between the player and a specific point on an entity.
     *
     * @param en The target entity to calculate pitch difference to
     * @param targetHeight The vertical offset from entity's feet to aim at (e.g., 1.62 for head)
     * @return The pitch difference in degrees
     */
    public static double pitchFromEntity(Entity en, float targetHeight) {
        return (mc.thePlayer.rotationPitch - pitchToEntity(en, targetHeight));
    }

    public static double pitchFromVec(Vec3 v) {
        return (mc.thePlayer.rotationPitch - pitchToVec(v));
    }

    /**
     * Calculates the pitch angle needed to aim at a specific point on an entity.
     *
     * @param en The target entity to aim at
     * @param targetHeight The vertical offset from entity's feet to aim at (e.g., 1.62 for head)
     * @return The pitch angle in degrees needed to aim at the target
     */
    public static float pitchToEntity(Entity en, float targetHeight) {
        double x = mc.thePlayer.getDistanceToEntity(en);
        double y = mc.thePlayer.posY - (en.posY + targetHeight);
        double pitch = (((Math.atan2(x, y) * 180.0D) / 3.141592653589793D));
        return (float) (90 - pitch);
    }

    public static float pitchToVec(Vec3 v) {
        double x = mc.thePlayer.getDistance(v.xCoord, v.yCoord, v.zCoord);
        double y = mc.thePlayer.posY - v.yCoord;
        double pitch = (((Math.atan2(x, y) * 180.0D) / 3.141592653589793D));
        return (float) (90 - pitch);
    }

    /**
     * Calculates the movement angle in radians based on x and z movement inputs.
     *
     * @param angle The base angle in degrees
     * @param xMovement The movement along the x-axis
     * @param zMovement The movement along the z-axis
     * @return The calculated movement angle in radians
     */
    public static float calculateMovementAngleRadians(float angle, float xMovement, float zMovement) {
        ...
        if (zMovement > 0.0f) {
            angle -= 90.0f * multiplier;
        } else if (zMovement < 0.0f) {
            angle += 90.0f * multiplier;
        }
        return angle * 0.017453292f; // Convert degrees to radians (PI/180)
    }
