package slate.utility._unused.aim;

import akka.japi.Pair;
import slate.utility._unused.anticheats.utils.phys.Vec2;
import slate.utility._unused.anticheats.utils.world.PlayerRotation;
...
public class AimSimulator {
    ...
    @Getter
    private Vec3 hitPos = new Vec3(0, 0, 0);
    ...
    public @NotNull Pair<Float, Float> getRotation(@NotNull EntityLivingBase target) {
        ...
        if (nearest) {
            ...
            if (MoveUtil.isMoving() || MoveUtil.isMoving(target))
                targetPosition = targetPosition.add(new Vec3(Utils.randomizeDouble(nearestAcc - 1, 1 - nearestAcc) * 0.4, Utils.randomizeDouble(nearestAcc - 1, 1 - nearestAcc) * 0.4, Utils.randomizeDouble(nearestAcc - 1, 1 - nearestAcc) * 0.4));
        } else {
            targetPosition = new Vec3((aimBox.maxX + aimBox.minX) / 2, aimBox.minY + target.getEyeHeight() - 0.15, (aimBox.maxZ + aimBox.minZ) / 2);
        }

        if (offset && offsetPre) {
            targetPosition = targetPosition.add(new Vec3(offsetX, offsetY, offsetX));
        }
        ...
        if (yDiff >= 0 && lazy) {
            ...
            if (!target.onGround && (MoveUtil.isMoving() || MoveUtil.isMoving(target)))
                targetPosition = targetPosition.add(new Vec3(0, Utils.randomizeDouble(lazyAcc - 1, 1 - lazyAcc) * 0.4, 0));
        }
        ...
        if (noise) {
            ...
            targetPosition = new Vec3(
                    normal(targetBox.maxX, targetBox.minX, targetPosition.xCoord + lastNoiseDeltaX),
                    normal(targetBox.maxY, targetBox.minY, targetPosition.yCoord + lastNoiseDeltaY),
                    normal(targetBox.maxZ, targetBox.minZ, targetPosition.zCoord + lastNoiseDeltaZ)
            );
        }
    ...
    public static float rotMove(double target, double current, double diff, Double gcd) {
        ...
        if ((float) target > (float) current) {
            ...
            if (dist1 > dist2) {  // moving from other side is closer
