...
public class RotationUtils {
    private static final Vec3 ZERO_POS = new Vec3(0, 0, 0);
    ...
    //     if (box == null) {
    //         return eyePos.distanceTo(Vec3.convert(blockPos)) > n;
    //     } else {
    ...
    @Contract("_, _ -> new")
    public static @NotNull Vec3 getNearestPoint(@NotNull AxisAlignedBB from, @NotNull Vec3 to) {
    ...
    public static EnumFacing getEnumFacing(@NotNull Vec3 hitPos, @NotNull AxisAlignedBB box) {
        if (hitPos.yCoord == box.maxY) {
            return EnumFacing.UP;
        } else if (hitPos.yCoord == box.minY) {
            return EnumFacing.DOWN;
        } else if (hitPos.xCoord == box.minX) {
            return EnumFacing.WEST;
        } else if (hitPos.xCoord == box.maxX) {
            return EnumFacing.EAST;
        } else if (hitPos.zCoord == box.minZ) {
            return EnumFacing.NORTH;
        } else if (hitPos.zCoord == box.maxZ) {
    ...
    public static @NotNull MovingObjectPosition rayCastStrict(final float yaw, final float pitch, final double range) {
        ...
        return new MovingObjectPosition(MovingObjectPosition.MovingObjectType.MISS, ZERO_POS, null, BlockPos.ORIGIN);
    }
